    ...
    wx.chooseImage({
      ...
      success: (res) => {
        ...
        if (this.data.imgList.length != 0) {
          this.setData({
            imgList: this.data.imgList.concat(res.tempFilePaths)//imgList是本地地址的存储 
          })
        } else {
          this.setData({
    ...
    if(!userinfo)
    {
      ...
      for (let i = 0; i < this.data.imgList.length; i++) {
        ...
        promiseArr.push(new Promise((reslove, reject) => {
          wx.cloud.uploadFile({
            cloudPath: 'liuyan/' + new Date().getTime() + suffix,
      ...
      Promise.all(promiseArr).then(res => {
        db.collection('timeline').add({
          data: {
            fileIDs: this.data.fileIDs,  //fileIDs是用户分享的图片
